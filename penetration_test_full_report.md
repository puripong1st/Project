...
**ระบบ**: RMUTP Door Access Control System  
...
**วิธีโจมตี**:
```bash
curl https://rmutp-door.vercel.app/api/esp32/status?room=CE-402
...
- ESP32 API Key: `rmutp-door-access-super-secret-key-2026-esp32`
- JWT Secret: `rmutp-door-access-super-secret-key-2024`
...
**รายละเอียด**:
```typescript
const char *api_key = "rmutp-door-access-super-secret-key-2026-esp32";
```
...
```javascript
sessionStorage.setItem('rmutp_qr_verified_CE-401', '1');  // bypass ได้ทันที
```
...
2. แก้ไข CORS header:
   ...
   - หรือใช้ allowlist เฉพาะ domain ที่ต้องการ เช่น:
     const allowedOrigin = process.env.NEXT_PUBLIC_APP_URL || "https://rmutp-door.vercel.app";
...
2. แทนที่ hardcoded API key:
   เดิม: const char *api_key = "rmutp-door-access-super-secret-key-2026-esp32";
...
1. แทนที่ localStorage สำหรับ bypass_token ด้วย sessionStorage:
   เดิม: localStorage.setItem("rmutp_temp_bypass_token", data.bypass_token);
   ใหม่: sessionStorage.setItem("rmutp_temp_bypass_token", data.bypass_token);

2. แทนที่ localStorage สำหรับ user session:
   เดิม: localStorage.setItem("rmutp_user_session", JSON.stringify(session));
   ใหม่: sessionStorage.setItem("rmutp_user_session", JSON.stringify(session));
...
1. แทนที่ "*" ใน Access-Control-Allow-Origin ทุกที่:
   const ALLOWED_ORIGIN = process.env.NEXT_PUBLIC_APP_URL || "https://rmutp-door.vercel.app";
